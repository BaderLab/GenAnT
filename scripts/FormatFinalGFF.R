...
toga_simple <- strsplit(symbols_simplify$toga_gene,";")
...
for(i in 1:length(liftoff_simple)) {
  ...
  if(!any(hier %in% c(tog,lift))) {
    simple[i] <- symbols_simplify$hierarchical_gene[i]
  }
  
  if(any(hier %in% lift) == FALSE & any(hier %in% tog) == TRUE) {
    out1 <- hier[hier %in% tog]
    simple[i] <- paste(out1, collapse = ";")
  } 
  
  if(any(hier %in% lift) == TRUE & any(hier %in% tog) == FALSE) {
    out1 <- hier[hier %in% lift]
    simple[i] <- paste(out1, collapse = ";")
  } 
  
  if(any(hier %in% lift) == TRUE & any(hier %in% tog) == TRUE) {
    out1 <- hier[hier %in% lift]
    out2 <-  hier[hier %in% tog]
    inter <- intersect(out1,out2)
    
    if(length(inter) > 0) {
      simple[i] <- paste(c(inter), collapse = ";")
      
    } else {
      simple[i] <- paste(c(out1,out2), collapse = ";")
