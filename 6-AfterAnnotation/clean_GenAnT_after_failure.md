...
GenAnT is a meta-pipeline that integrates many tools and may fail for one reason or another. This markdown describes which intermediate files should be deleted before restarting the run, and why.
...
The hal to chain conversion may occur with an empty input file. In this instance, a chain file is formed without any acutal alignment info. These files are very small (typically >1kb). The below script checks the filesize of the chain file and deletes it if it's under 500kb
...
If the script failed before the `mikado_prepare` step: Delete empty GFF files to ensure their generation is rerun.
