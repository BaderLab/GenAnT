...
rule mask_assembly:
    ...
    threads:
        36  # Request 16 threads for this rule
    params:
        memory_per_thread="8G",
        extra_cluster_opt="",
        walltime="150:00:00"
...
rule mikado_prepare:
    ...
    shell: 
        """
            scripts/12_mikado_configure_and_prepare.sh {config[outDir]} {config[externalDir]} {config[customRef]} {config[liftoffRef]} {workflow.basedir} {[mikadoScore]}
        """
