...
if [[ $(ls -A $outDir/RNAseq_alignment | wc -l) -gt 0 && $(ls -A $outDir/ISOseq_alignment | wc -l) -gt 0 ]] ; then
	...
	samtools merge -@ $threads $outDir/transcript_selection/merged.sr.bam $RNA_aln/*merged.bam
	samtools index $outDir/transcript_selection/merged.sr.bam

	portcullis full -t $threads $ASSEMBLY $outDir/transcript_selection/merged.sr.bam -o $outDir/transcript_selection


	ISO_aln=$outDir/ISOseq_alignment
	samtools merge -@ $threads $outDir/transcript_selection/merged.lr.bam $ISO_aln/*merged.bam
