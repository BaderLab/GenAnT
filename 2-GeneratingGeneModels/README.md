...
- In our experience, TOGA errors are most commonly associated with one of four sources.
  ...
  5) In our experience, if TOGA reaches the stage where it generates and submits many nextflow jobs simultaneously, then you have set everything up. We have had an array of errors come up after this stage, but they have all come down to there being a memory or threading error in at least one stage. For example, CESAR merge has crashed for us with a "corrupted fasta" and 28 threads, but the identical job functioned at 32 threads. Thereofore, if TOGA passes pre-flight checks and sucessfully submits nexflow jobs before failing, we reccomend increasing memory/threads and re-submitting before spending loads of time debungging.
